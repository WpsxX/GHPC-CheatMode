# CheatMode 与四个参考 mod 的差异对比报告（基准：各仓库已提交源文件）

> 对比对象（仅两方）：
> - **当前改动（分析主体）**：`CheatMode` v1.6.0（本仓库 `CheatMode/` 下的源码：`CheatMode.cs`、`CheatAmmoPatches.cs`、`CheatFireSupportPatches.cs`、`CheatInvinciblePatches.cs`、`CheatESPPatches.cs`、`Render.cs`）。
> - **仓库源文件（基准，远端/已提交版本）**：以下四个 GitHub 仓库默认分支源文件（2026-09-08 快照），各锚定到其默认分支提交：
...
- 每个 mod 一节的“差异比对结果”表即为该映射的产出；第 5 节为其汇总。

---

## 1. 与 GHPC_Artillery_Rework 的对比（火炮/火力支援）— 基准 `GHPC_Artillery_Rework.cs` v1.1.1

**文件映射**：上游单文件 `GHPC_Artillery_Rework.cs` ↔ CheatMode `CheatFireSupportPatches.cs`（+`CheatMode.cs` 中火炮设置/常量）。净变化：**重写 + 扩写（作弊向裁剪 + 新增多项能力）**。

### 1.1 目标方法
- **Artillery Rework（已提交版）**：拦截 `ArtilleryBattery.SendFireMission`，通过 **Prefix 改写方法入参**（`delaySeconds / roundCount / radiusMeters / secondsBetweenRounds`，`ref`）实现“乘数”；另补丁 UI 层（`MapIconControlType.UpdateCooldownDelayResponse`、`MapFireSupportPanel.OnClickSupportButton`、`FireMissionManager.SendFireMissionPlanned`、`MapController.InitControlState`、`DynamicMissionLauncher.LaunchDynamicMission`）与 `AlertHud`。
- **CheatMode**：拦截 **`SendFireMission` 的 Postfix**，改写“本次齐射的当前值字段”（`_currentShotQuota/_currentInterShotTime/_currentRadius/TimeUntilImpactSeconds/RemainingDelay`）；拦截 **`DoSingleShot`** 做阵营伤害；补丁 `DoUpdate` 做无冷却。不动 UI 层。

### 1.2 差异比对结果（功能级）
| 上游 v1.1.1 功能点 | 对应文件/方法 | CheatMode 净变化 |
|---|---|---|
| 弹量 volume multiplier | 无同名项（用 `VolleyRounds` 固定发数） | **重写**（-1=原版 / 1~128 固定发数） |
| time-to-target multiplier | `ArtilleryTimeToTarget` 比例 + 首发延迟取消 + 秒抵达 | **重写**（含上游没有的“整轮同帧落”） |
| accuracy multiplier | `ArtilleryAccuracy` 比例 + 零散布 | **重写**（语义从“除以乘数”改为“比例缩放”） |
| HE/Smoke/Illu × OnCall/Planned 18 路乘数 | — | **无对应**（刻意裁剪：统一参数） |
| `CallerDetector` 栈回溯区分 OnCall/Planned | — | **无对应**（改用 `ignoreCooldown` + 阵营数组判定） |
| 中途取消炮击（UI 点击） | — | **无对应**（作弊场景不需要） |
| `AlertHud` 来袭警报 + 随机语音 | — | **无对应**（纯配置，无提示） |
| — | `InfiniteFireSupport`（恢复 `_missionsAvailable`） | **新增**（上游无） |
| — | `FireSupportNoCooldown`（`DoUpdate` 清零 / CAS setter 拦截） | **新增**（上游无） |
| — | `ArtilleryFactionAwareEnabled` 阵营伤害（`DoSingleShot`） | **新增**（上游无） |
| — | 秒抵达同帧补足整轮并顶高 `_shotCounter` | **新增**（面板防卡 Incoming） |

### 1.3 相对上游的修复/改进
1. **不污染阵地模板**：上游直接缩放入参，曾出现“乘数平方化”累乘 bug（其 changelog 自述修复过）；CheatMode 只改写每呼叫重新赋值的“当前值字段”，不改模板字段（`_shots/_interShotDelaySeconds/_randomDispersionRadiusMeters/_onCallImpactDelay`），机制上杜绝“越用越快/准”。
2. **不干扰剧情脚本**：`ignoreCooldown==true` 的临时/脚本阵地一律放行原版；上游依赖栈回溯判断，隔离性较弱。
3. **面板状态正确性**：秒抵达时同帧补足整轮并把 `_shotCounter` 顶到配额，面板不会卡 “Incoming”。
4. **无冷却的实现**：明确“不拦 `RemainingCooldown` 写入以免破坏 `CooldownManager` 面板”，改为 `DoUpdate` 后置清零。

---

## 2. 与 InfiniteAmmo 的对比（无限弹药）— 基准 `InfiniteAmmo.cs` v1.0.1
...
## 3. 与 Invincible-Tank 的对比（无敌）— 基准 `Invincible_Tank.cs` v1.0.0
...
|---|---|---|
| `PlayerFactionStore`（一次性字符串） | `CheatModeMod.ResolvePlayerFaction()` + `CheatDamageFilter` | **重写**：权威阵营级联 + 每事件实时解析 |
...
## 4. 与 GHPCESP 的对比（ESP）— 基准 `src/GHPCESPMod.cs` 等 v1.2.0
...
### 4.3 相对上游 GHPCESP 的修复/改进
空引用防御、按键收敛（两种 ESP 合并到 F8）、标签带距离、自清理循环统一。

---

## 5. CheatMode 相对四个上游的整体净变化汇总

| 子系统 | 上游基准 | 净变化 |
|---|---|---|
| 火炮/火力支援 | Artillery Rework v1.1.1 | 重写 + 扩写：新增无限次数/无冷却/阵营伤害/秒抵达齐射；裁剪弹种与 Planned 细分、取消功能、HUD 语音 |
...
- **无** Artillery Rework 的 HUD 来袭警报/随机语音、HE/Smoke/照明 × OnCall/Planned 18 路配置、中途取消炮击。
- **无** GHPCESP 的 F8/F9 双 ESP 独立开关（CheatMode 刻意合并）。
- 属作弊/单机向：未做多人公平性处理；AAR 只对炮兵即时齐射显式回避。
